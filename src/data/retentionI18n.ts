...
> = {
  ...
  "Annual Coverage Reviews Completed": {
    label: { en: "Annual Coverage Reviews Completed", kr: "연간 보장 검토 완료" },
    sub: { en: "Documented this folio period", kr: "이번 폴리오 기간 기록" },
  },
  "Life Referrals": {
...
const combinedKpiLabels: Record<string, Record<RetentionLocale, string>> = {
  ...
  "Annual Coverage Reviews": { en: "Annual Coverage Reviews", kr: "연간 보장 검토" },
...
export function getRetentionCopy(locale: RetentionLocale) {
  ...
  return {
    ...
    agencyZoom: {
      section: isKr
        ? "AgencyZoom 파이프라인: 유지 레이어 (Arminda 운영 지원)"
        : "AgencyZoom Pipelines: Retention Layer (Arminda: Operations Support)",
      alertTitle: isKr ? "Arminda 역할 명확화" : "Arminda's Role Clarification",
      alertBody: isKr
        ? "Arminda Sumido는 AgencyZoom 운영, 백오피스 데이터 입력, 운영 지원을 담당합니다. 파이프라인 설계나 워크플로 소유권은 없습니다."
        : "Arminda Sumido supports AgencyZoom operations, back-office data entry, and operations support. She does not own pipeline setup or workflow design.",
      headers: isKr
